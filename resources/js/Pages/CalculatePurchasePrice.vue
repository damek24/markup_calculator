<template>
    ...
    <div class="border my-4 p-3 rounded-3" v-if="sellPrice && (margin || markup)">
        ...
        <table class="table table-striped table-bordered">
            ...
            <tr>
                ...
                <td v-if="marginOrMarkup === 2">
                   {{ sellNetPrice }} / ({{ markup }} / 100 + 1)
                </td>
                <td v-else>
                    {{ sellNetPrice }} -  {{ sellNetPrice }} * {{ margin }} / 100
                </td>
                <td v-if="marginOrMarkup === 2">
                    ...
                    <code :class="markupProof ? 'd-block' : 'd-none'">
                        ...
                        cena sprzedaży = narzut / 100 * cena zakupu + cena zakupu <br/>
                        cena sprzedaży = cena zakupu * (narzut / 100 + 1) <br/>
                        cena zakupu = cena sprzedaży / (narzut / 100 + 1)
                    </code>
                    <span class="d-block">
                        cena zakupu = cena sprzedaży / (narzut / 100 + 1)
                    </span>
                ...
                <td v-else>
                    ...
                    <code :class="marginProof ? 'd-block' : 'd-none'">
                        ...
                        cena zakupu = cena sprzedaży - marża / 100 * cena sprzedaży
                    </code>
                    <span class="d-block">
                        cena zakupu = cena sprzedaży - marża / 100 * cena sprzedaży
                    </span>
...
const calculateBuyMargin = (): void => {
    buyNetPrice.value = round(sellNetPrice.value - margin.value / 100 * sellNetPrice.value)
    buyGrossPrice.value = round(buyNetPrice.value * (1 + (vat / 100)));
    marginResult = margin
}
const calculateBuyMarkup = (): void => {
    buyNetPrice.value = round( sellNetPrice.value / ( markup.value / 100 + 1))
    buyGrossPrice.value = round(buyNetPrice.value * (1 + (vat / 100)));
    markupResult = markup
}
